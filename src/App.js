      ...
      <div style={{ padding:"32px 28px 20px",position:"relative" }}>
        ...
        <div style={{ marginTop:36 }}>
          ...
          <h1 style={{ fontSize:36,fontWeight:800,color:B.white,lineHeight:1.1,
            ...
            {mode==="login"?"welcome back.":"protect your business."}
          </h1>
...
function BottomNav({ active, go }) {
  return <div style={{ position:"fixed",bottom:0,left:0,right:0,zIndex:100,
    ...
    boxShadow:"0 -4px 20px rgba(13,27,62,.08)",maxWidth:430,margin:"0 auto" }}>
    {TABS.map(t=>{
...
// ── top bar ───────────────────────────────────────────────────────────────────
function TopBar({ user, go, onLogout }) {
  ...
  return <div style={{ position:"sticky",top:0,zIndex:50,
    ...
    <div style={{ display:"flex",alignItems:"center",gap:10 }}>
      ...
      <button onClick={()=>setMenu(m=>!m)} style={{ width:34,height:34,borderRadius:"50%",
    ...
    {menu && <div style={{ position:"fixed",top:60,right:20,background:B.white,
      ...
      <button onClick={()=>{ go("profile"); setMenu(false); }} style={{ display:"block",width:"100%",
        ...
        fontSize:14,color:B.ink,cursor:"pointer",borderRadius:10 }}>⚙ account settings</button>
      <button onClick={()=>{ go("pricing"); setMenu(false); }} style={{ display:"block",width:"100%",
        textAlign:"left",padding:"10px 14px",background:"none",border:"none",
        fontSize:14,color:B.purple,cursor:"pointer",borderRadius:10 }}>✦ upgrade plan</button>
      <button onClick={()=>{ setMenu(false); onLogout(); }} style={{ display:"block",width:"100%",
        textAlign:"left",padding:"10px 14px",background:"none",border:"none",
        fontSize:14,color:B.red,cursor:"pointer",borderRadius:10 }}>→ log out</button>
    </div>}
  ...
  if(view==="detail"&&sel) return <div style={{ padding:"20px 20px 100px" }}>
    ...
    <Card>
      ...
      {e.checks.map(item=><div key={item.id} style={{ display:"flex",alignItems:"center",gap:11,
        ...
        <button onClick={()=>onToggle(e.id,item.id)} style={{ width:22,height:22,borderRadius:6,flexShrink:0,
...
export default function App() {
  ...
  return <div style={{ maxWidth:430,margin:"0 auto",minHeight:"100vh",background:B.bg,fontFamily:FS,position:"relative" }}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=DM+Mono:wght@400;500;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}button{-webkit-tap-highlight-color:transparent}button:active:not(:disabled){transform:scale(.97)}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}::-webkit-scrollbar{display:none}textarea,select,input{-webkit-appearance:none;appearance:none}textarea:focus,select:focus,input:focus{border-color:${B.blue}!important;box-shadow:0 0 0 3px ${B.blue}18!important;outline:none}html{-webkit-text-size-adjust:100%}`}</style>
    <TopBar user={user} go={go} onLogout={onLogout}/>
